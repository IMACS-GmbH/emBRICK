...
#define BRICKBUS_STACK_VERSION	6
...
#define BRICKBUS_CHECKSUM		0x55				//Das Checksummenbyte wird so angepasst, dass alle Bytes zusammengezaehlt diesen Wert ergeben (Bei Master zB. AdressByte+Kommandobyte+Checksummenbyte=BRICKBUS_CHECKSUM
...
// max Time between 2 transactions (complete sequence) on the brickBUS in master synchronous mode. Value in ms
#ifndef BB_MAX_GAP
#define	BB_MAX_GAP				40
#endif
...
typedef struct brickBUS_Data{
	unsigned char	syncronousTrigger;				// in synchronousMode, set to trigger the next transaction on the brickBUS, reset by brickBUS Stack
	unsigned char	laenge;							// length of actual data package (max of Rx and Tx)
    unsigned char   fehlVersuche;					// counts retries on wrong checksum
...
typedef struct brickBUS_Slave{  
	...
	unsigned short	totalfailCounter ;			    // counts all failed transactions since last BUSInit (for debugging purposes)
}BB_SLAVE;
...
typedef struct brickBUS_Bus{     
    ...
    unsigned char	synchronousMode;				// if set, a brickBUS transaction is triggered by the application (used by remote master applications). After a timeout, a transaction will take place in every case.
	unsigned char	transactionfailed;				// set during cycle (requesting all slaves) if a transaction failed
    unsigned short	lengthTimeout;					// Timeout - Factor : effective Timeout is (Factor + 1) * 30us + 16us
	char			LastInitFail ;					// last error returned by InitbBUS(). Set to 0 while no error ever has occured
	unsigned short	NumOfSucceededInits ;			// Number of successfull Inits since PowerUp, can be used for debugging purposes
	unsigned short	NumOfFailedInits ;				// Number of failed Inits since PowerUp, can be used for debugging purposes
}BB_BUS;
...
typedef struct brickBUS
{
	...
	unsigned short	bBMOSIBufferHandshake ;				// Handshake for bBMOSIBuffer acces. Incremented on beginning and end of all writes to Buffer, uneven value := writing access under way
	unsigned short	bBMISOBufferHandshake ;				// Handshake for bBMISOBuffer acces. Incremented on beginning and end of all writes to Buffer, uneven value := writing access under way
    BB_BUS			bB_BUS;
