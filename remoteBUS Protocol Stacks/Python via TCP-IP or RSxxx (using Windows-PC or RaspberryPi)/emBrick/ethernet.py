...
class emBrickConnection:
    ...
    def start_ethernet(self):
        ...
        for i in range(len(self.ipList)):
            try:
                ...
                print('Connecting with the given Ip Adress:', self.ipList[i], ' failed.')
                del self.ipList[i]
                del self.nodes[i]
    ...
    def update():
        ...
        with lock:
            ...
            connect.brick_input()
        threading.Timer(connect.updateRate, connect.update).start()
        ...
        with lock:
            try:
                ...
                try:
                    ...
                    self.gS = "Byte Position not found!"
            except:
...
def init():
    ...
    for n in connect.nodes:
        print(
            f'\nNode {n + 1} with {connect.ipList[n]} is connected with the local master: '
            f'{local.id[n - 1] // 1000}-{local.id[n - 1] % 1000}')
